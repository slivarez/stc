...
#if ($usver=~m/[^\_\-\.a-z0-9]/) {print "User name bad <b>$usver</b><br>";}

$o=$query->param(otdel);
$back_url=$query->param('back_url');
$back_url1=$query->param('back_url1');
if (!safe_url($back_url, 'st.cgi')){goto l1;}
...
#   print "<td><input type=text name=user1 size=20 value=\"".($usver ne ''?$usver:$start_usver)."\"></td>";
    print "<tr>
	    <td>$msg[12]:</td>
...
#print "<tr>";
#print "<td>$msg[16]:</td>";
#print "<td><select name=otd>";
#foreach (@otdels)
#          {
#          print "<option value=\"$_\"";
#            if ($o eq $_) {print " selected";}
#          print ">$otdel_prn{$_}</option>";
#          }
#print "</select></td>";
#print "</tr>";
    ...
    print "<input type=hidden name=start_user value=\"$selected_user\">
           ...
           <input type=hidden name=back_url1 value=\"$back_url1\">
         <tr align=center>
	<td><input type=submit value=\"$msg[26]\"></td>
...
#####################  Bottom Back  #####################
    ...
    <form action=\"$back_url\" method=\"POST\">
           ...
           <input type=hidden name=back_url value=\"$back_url1\">
           <input type=hidden name=otdel value=\"$o\">
...
if ($usver ne ''){
	...
	open(LOG, "$conf_access_log.m$mm") || print "$msg[2] $conf_access_log.m$mm: $!<br>";
        while(<LOG>)
        {
    	    ($user_name, $size,$foo,$skip_cache)=log_line($_);
	    $traffic{$realuser{$user_name}}{$skip_cache} += $size;
	    $usage{$realuser{$user_name}}{$site}{$skip_cache} += $size;
     	}#while
	close(LOG);
print "MM:$mm MON:$mon<br>";
	if ($mm == $mon){
	...
	$tr=$traffic{$usver}{"1"}/$conf_mega_byte;
	$tr_0=$traffic{$usver}{"0"}/$conf_mega_byte;
	$tr_1=$traffic{$usver}{"-1"}/$conf_mega_byte;
    	$tr_2=$traffic{$usver}{"-2"}/$conf_mega_byte;
	...
	print ("<tr><td align=left colspan=2 style=\"font-size:10px;\"><b>Download:</b><br></td></tr>");
	...
	foreach $site (sort { $ipusage{$b}{"1"} <=> $ipusage{$a}{"1"} } keys %ipusage)
	{
		...
		unless ($print)
		{
		    ...
		    print "</tr>";
		    $print++;
		    print "<tr><td align=left colspan=2 style=\"font-size:10px;\"><b>No Download:</b><br></td></tr>";
    		    print "<tr align=center><th class=stat>$msg[117]</th>";
		    print "<th nowrap class=stat>EXCLUDE</th>";
		    print "<th nowrap class=stat>CACHE</th>";
		    print "<th nowrap class=stat>Download</th>";
