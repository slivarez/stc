...
print "<center>";
#Security check
if (!safe_url($back_url, 'mess_view.cgi')){goto l1;}
#end of security check

if (($conf_web_messages eq 'admin') or ($conf_web_messages eq 'all')){
    print "$msg{mess_for} <b>$user</b><br><br>";
    ...
    print "<input type=hidden name=back_url value=mess_view.cgi>";
    print "<input type=submit value=\"$msg[81]\">";
    print "</form>";
	...
	print "<form action=\"mess_msend.cgi\" method=\"POST\">";
        ...
        print "</form>";
	print "<form action=\"mess_admin.cgi\">";
        print "<input type=submit value=\"$msg[58] Administrator\">";
	print "</form><br>";
    }
    if ($mess_kill eq "yes"){
...
for ($i=$mess_count; $i>=1; --$i){
    ...
    if ($admin{$user} eq 'yep'){
	print "<a href=\"profile.cgi?user=$messages[$i]{from}&back_url=mess_view.cgi\">$messages[$i]{from}</a>";
    }else{
    ...
    print "</td>";
    printf ("<td class=\"mess_head\" bgcolor=$conf_web_mes_head_color><b>$msg[62]: </b>%02d.%02d.%04d<br><b>$msg[82]: </b>%02d:%02d:%02d</td>",$dd,$mm,$yy,$hh,$mi,$ss);
    print "</tr>";

    print "<tr>";
    print "<td class=\"mess_head\" colspan=2 bgcolor=$conf_web_mes_head_color><b>$msg[63]: </b>$messages[$i]{tema}</td>";
    print "</tr>";
